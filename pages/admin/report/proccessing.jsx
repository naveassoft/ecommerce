...
const DProccessing = () => {
  ...
  async function getAllOrder() {
    const { data, error } = await store?.fetchData(
      `/api/order?status=processing&limit=${limit}&page=${page}`
    );
    if (data) {
      setOrders(data.data);
      setCount(data.count);
  ...
  async function getOrderbyDate(payload) {
    const { data, error } = await store?.fetchData(
      `/api/order?date=true&status=processing&start=${payload.start_date}&end=${payload.end_date}&limit=${limit}&page=${page}`
    );
    if (data) {
      setOrders(data.data);
      setCount(data.count);
